...
class _DrawerContentState extends ConsumerState<DrawerContent> {
  ...
  ///
  /// Shows language selector modal and calls setSDKLanguage API when language is selected.
  /// The global SDKEventProvider handler will update the languageProvider on success.
  Future<void> _handleChangeLanguage() async {
    print('DrawerContent - _handleChangeLanguage called, mounted: $mounted');
    final languageState = ref.read(languageProvider);

    // Capture ScaffoldMessenger BEFORE showing modal (to avoid unmounted context errors)
    print('DrawerContent - Capturing ScaffoldMessenger and Navigator...');
    final scaffoldMessenger = ScaffoldMessenger.of(context);
    final rootNavigator = Navigator.of(context, rootNavigator: true);
    print('DrawerContent - Captured successfully');
    ...
    showLanguageSelector(
      ...
      onSelectLanguage: (language) async {
        print('DrawerContent - Language selected: ${language.displayText}');

        // Close modal using captured navigator
        rootNavigator.pop();
        ...
        try {
          print('DrawerContent - Calling setSDKLanguage API: ${language.displayText} (${language.lang})');

          final rdnaService = RdnaService.getInstance();

          // Call setSDKLanguage API
          // The global SDKEventProvider handler will handle the onSetLanguageResponse event
          // and update the languageProvider automatically
          ...
          if (response.error?.longErrorCode == 0) {
            // Sync success - async event will fire and SDKEventProvider will update languageProvider
            print('DrawerContent - SetSDKLanguage sync success, waiting for async event');

            // Show success message (use captured scaffoldMessenger to avoid unmounted context)
  ...
  /// Build Change Language menu item with current language display
