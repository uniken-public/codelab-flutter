...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  Future<void> _handleLogout() async {
    ...
    final response = await rdnaService.logOff(_userId!);

    print('DashboardScreen - LogOff sync response received');
    print('  Long Error Code: ${response.error?.longErrorCode}');

    if (response.error?.longErrorCode == 0) {
      print('DashboardScreen - LogOff successful, waiting for onUserLoggedOff event');
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response.error?.errorString ?? 'Logout failed'),
